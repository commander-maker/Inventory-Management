...
<<<<<<< HEAD
        const [deliveryRes, inventoryRes, userRes, incomeRes, recentTransRes] = await Promise.all([
          deliveryAPI.getAll(),
          inventoryAPI.getAll(),
          userAPI.getAll(),
          financeAPI.getIncome(),
          financeAPI.getRecentTransactions(4)
        ]);
        ...
        const recentTransData = recentTransRes.data.data || [];
        
        // Convert RecentTransaction records to display format
        const formattedTransactions = recentTransData.map((trans, index) => {
          const timeAgo = getTimeAgo(trans.createdAt);
          const productDisplay = trans.quantity ? `${trans.productName} (${trans.quantity})` : trans.productName;
          
          return {
            id: `trans-${trans.id}`,
            type: trans.type.toLowerCase(),
            product: productDisplay,
            amount: `LKR ${parseFloat(trans.amount).toLocaleString()}`,
            time: timeAgo,
            person: trans.customerName || trans.agentName || 'Customer',
            icon: getTransactionIcon(trans.productName),
            color: getTransactionColor(trans.productName),
            iconColor: getTransactionIconColor(trans.productName),
            status: trans.status,
            isLatest: index === 0, // Mark first transaction as latest
